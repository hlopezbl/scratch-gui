 ...
 */
let extensionURL =
    "https://champierre.github.io/ml2scratch/dist/ml2scratch.mjs";
...
const Message = {
    ...
    set_classification_interval: {
        ...
        "ja-Hira":
            "ラベルづけを[CLASSIFICATION_INTERVAL]びょうかんに1かいおこなう",
    ...
    first_training_warning: {
        ...
        "ja-Hira":
            "さいしょのがくしゅうにはしばらくじかんがかかるので、なんどもクリックしないでください。",
...
class Scratch3ML2ScratchBlocks {
    ...
    download() {
        ...
        downloadWindow.document.write(`
            ...
            <html lang="en">
            <head>
                <title>${Message.download_learning_data[this.locale]}</title>
                <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/tailwindcss/2.2.19/tailwind.min.css">
            </head>
            <body>
                <div class="container mx-5 py-5">
                    <h1 class="text-xl ">${
                        Message.download_learning_data[this.locale]
                    } to system</h1>
                    <div class="overflow-hidden relative w-64 mt-4 mb-4">
                        <button
                            id="downloadToSystem"
                            class="bg-blue-500 hover:bg-blue-light text-white font-bold py-2 px-4 w-full inline-flex items-center"
                        >
                    ...
                    <div class="overflow-hidden relative w-64 mt-4 mb-4">
                        ...
                        >
                            ...
                            <span class="ml-2">Save to Cloud</span>
                        </button>
                    </div>
                </div>
            </body>
        ...
        downloadWindow.document.getElementById("downloadToSystem").onclick =
            () => {
        ...
        downloadWindow.document.getElementById("downloadToCloud").onclick =
            async () => {
                const projectId = window.location.hash.substring(1) || null;

                if (projectId) {
                    const mlModelData = await this.knnClassifier.save(
                        null,
                        true
                    );
                    ...
                    const API_URL =
                        process.env.NODE_ENV === "production"
                            ? `https://scratch.brilliantlabs.ca/api/mlm/${projectId}`
                            : `http://localhost:4000/api/mlm/${projectId}`;

                    // PUT the ML model data to the cloud
                    ...
                    // console.log(responseResolved); // for testing
                } else {
    ...
    upload() {
        ...
        uploadWindow.document.write(`
            ...
            <html lang="en">
            <head>
                <title>${Message.upload_learning_data[this.locale]}</title>
                <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/tailwindcss/2.2.19/tailwind.min.css">
            </head>
            <body>
                <div class="container mx-5 py-5">
                    ...
                    <div class="overflow-hidden relative w-64 mt-4 mb-4">
                        <button class="bg-blue-500 hover:bg-blue-light text-white font-bold py-2 px-4 w-full inline-flex items-center">
                            ...
                            <span class="ml-2">${
                                Message.upload[this.locale]
                            }</span>
                        </button>
                        <input class="cursor-pointer absolute block py-2 px-4 w-1/ opacity-0 right-0 top-0" type="file" id="upload-files">
                    </div>

                    <br class="my-2" />

                    <h1 class="text-xl ">Load From Cloud</h1>
                    <button class="bg-blue-500 py-2 my-2 text-white w-64" id="cloudSelect">Load Model</button>
                </div>
            </body>
            </html>
        `);
        ...
        uploadWindow.document.getElementById("upload-files").onchange = () => {
            this.uploadButtonClicked(uploadWindow);
        };

        uploadWindow.document.getElementById("cloudSelect").onclick = () => {
            const projectId = window.location.hash.substring(1) || null;
            if (projectId) {
                this.loadMlModel(projectId, uploadWindow);
            } else {
                alert("No project ID found");
                uploadWindow.close();
            }
    ...
    async loadMlModel(projectId, uploadWindow) {
        const API_URL =
            process.env.NODE_ENV === "production"
                ? `https://scratch.brilliantlabs.ca/api/mlm/${projectId}`
                : `http://localhost:4000/api/mlm/${projectId}`;
    ...
    uploadButtonClicked(uploadWindow) {
        const files =
            uploadWindow.document.getElementById("upload-files").files;
